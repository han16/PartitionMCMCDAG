...
This function returns the incidence matrix and its corresponding edges 

## Neighbors for every move type and update the ancestors 
...
structureMCMC <- function(n,incidence,iterations,stepsave,fan.in,parenttable,scoretable,revallowed,moveprobs){
  ...
  currentDAGlogscores<-DAGnodescore(incidence, n, c(1:n)) # count the number of parents 
...
### 2.) number of neighbour graphs obtained by edge additions    1- E(i,j) - I(i,j) - A(i,j)
  ...
  add <- emptymatrix
  add[inter_add] <- 1
  add[,which(colSums(incidence)>fan.in-1)] <- 0  ### what is here ? subject to maxparents constraint 

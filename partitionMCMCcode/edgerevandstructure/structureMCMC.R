...
structureMCMC <- function(n,incidence,iterations,stepsave,fan.in,parenttable,scoretable,revallowed,moveprobs){
  ...
  currentDAGlogscores<-DAGnodescore(incidence, n, c(1:n)) # score directly DAGnodescore is in scoring/scorefns.R
...
# first ancestor matrix
  ancest1 <- ancestor(incidence)    # ancestor is in structurefns.R
...
### 2.) number of neighbour graphs obtained by edge additions    1- E(i,j) - I(i,j) - A(i,j)
  fan.in=maxparents
  inter_add <- which(fullmatrix - Ematrix - incidence - ancest1 >0) # why need to -ancest1 ??
  add <- emptymatrix
  add[inter_add] <- 1
  add[,which(colSums(incidence)>fan.in-1)] <- 0  ### what is here ?
